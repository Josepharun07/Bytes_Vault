...
// 6. EVENT LISTENER: Open Modal
if (addProductBtn) {
    addProductBtn.addEventListener('click', () => {
        modal.style.display = 'block';
        form.reset();
        specsContainer.innerHTML = ''; 
        addSpecField(); // Add default row
...
if (form) {
    form.addEventListener('submit', async (e) => {
        ...
        try {
            // Need token for Auth? If middleware is active, uncomment header:
            // const headers = { 'Authorization': `Bearer ${token}` }; 
            // Note: FormData handles Content-Type automatically, don't set it manually!
